...
import { createGameFBMNoise } from './game/fbm-generator.js';
...
function makeFBM(time = 0, offSeed = 0, xSize = 5, ySize = 5, scale = 5.0, minActive = 3, maxActive = 22) {
    let res = createGameFBMNoise(time, offSeed, xSize, ySize, scale, minActive, maxActive);
    currentSeed = res.offSeed; // update current seed for next call
    return res.fbm;
}

let currentSeed = Math.floor(Math.random() * 1000);
let startFbm = makeFBM(0, currentSeed);
...
export function createLoading() {
    ...
    const material = new ShaderMaterial({
        ...
        uniforms: {
            ...
            // uNoise: { value: makeFBM(0, 0) }, 
            uNoise2: { value: startFbm }, 
    ...
    function update(deltaTime) {
        ...
        t += deltaTime * speed;
        // setUniform('uNoise', makeFBM(t));
        setUniform('uNoise2', makeFBM(t, currentSeed));
    }
