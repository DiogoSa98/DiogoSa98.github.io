...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    float MAX_BOUNCE = 8.0;  // 5
    float MAX_DISPERSE = 8.0; // 3

    vec3 rayDirDisp, rayOriginDisp;
    // first rays with normal shading
    float hitDist = map(origin, rayDir, hitPos, nor); 
    if (hitDist > 0.) {
        ref = reflect(rayDir, nor);
        sam += light(hitPos, ref) * .00125;
        sam += pow(max(1. - abs(dot(rayDir, nor)), 0.), 5.) * .001; 
        ...
        raf = refract(rayDir, nor, 1./ior); 
        bool tif = raf == vec3(0); // total internal reflection 
        rayDirDisp = tif ? ref : raf; 
        rayOriginDisp = hitPos + 1e-5 * rayDirDisp; 
    } 
    ...
    for (float disperse = 0.; disperse < MAX_DISPERSE; ++disperse) { 
        ...
        for (float bounce = 0.; bounce < MAX_BOUNCE; bounce++) { 
            ...
            // entering = !entering; // TESTING
            float eta = entering ? 1. / ior : ior;
            nor = dot(rayDir, nor) < 0. ? nor : -nor; // !!! skipping this keeps the ray inside geometry creating cool weird patterns...
            
            extinctionDist += hitDist;
            
            ref = reflect(rayDir, nor); 
            // ref = reflect(-rayDir, nor); // TESTING
            ...
            // raf = mix(ref, raf, 0.9995); // TESTING
            bool tif = raf == vec3(0); // total internal reflection 
            rayDir = tif ? ref : raf; 
